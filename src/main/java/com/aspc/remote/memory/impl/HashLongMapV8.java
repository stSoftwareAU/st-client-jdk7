 ...
 *  @since       5 May 2010
 */
public final class HashLongMapV8 implements HashLongMap, Cloneable
{
    ...
    private static final Log LOGGER = CLogger.getLog( "com.aspc.remote.memory.impl.HashLongMapV8");//#LOGGER-NOPMD
    ...
    public HashLongMapV8(final int initialCapacity)
    {
     ...
     */
    @Override
    @SuppressWarnings({"AccessingNonPublicFieldOfAnotherObject", "AssertWithSideEffects", "CloneDoesntCallSuperClone"})
    public Object clone() throws CloneNotSupportedException//NOPMD
    {
        ...
        try
        {
            ...
            if( table == null)
            {
                HashLongMapV8 tempMap = new HashLongMapV8(ensureCapcity);
            ...
            HashLongMapV8 tempMap = new HashLongMapV8(table.length);
            ...
            if( keyArray != null)
            {
                ...
                if( overflowKey != null)
                {
                    if( last > overflowKey[0])
                    {
                        ...
                        nextStatus &= ( 0xffff ^ MASK_OVERFLOW_LARGER);
                    }
                    else
                    {
                        nextStatus |= MASK_OVERFLOW_LARGER;
                    }
                }

                if( sorted )
                {
                    nextStatus |= MASK_SORTED;
                }
                else
                {
                    nextStatus &= ( 0xffff ^ MASK_SORTED);
                }

                nextStatus &= ( 0xffff ^ MASK_UNKNOWN_SORT);
    ...
    private long[] noLockGetSortedKeyArray()
    {
        ...
        if((tmpStatus & MASK_SORTED) != MASK_SORTED)
        {
            ...
            {
                list = list.clone();
                nextStatus &= ( 0xffff ^ MASK_ARRAY_SHARED);
            }
    ...
    private void rehash(int adjustCount)
    {
        ...
        for (Entry base: oldMap)
        {
            for (Entry old = base ; old != null ; )
            {
                Entry e = old;
                old = old.next;
                ...
                if( count != 0)
                {
                    ...
                    if( value == tmpValue)
                    {
                        ...
                        if( overflowKey== null)
                        {
                            ...
                            int tmpStatus = status.get();
                            int nextStatus = tmpStatus;
                            nextStatus |= MASK_OVERFLOW;
                            nextStatus &= ( 0xffff ^ MASK_FULL_LIST);
                            if( (tmpStatus & MASK_SORTED) == MASK_SORTED)
                            {
                                ...
                                {
                                    nextStatus |= MASK_OVERFLOW_SMALLER;
                                }
                            }
    ...
    public void putMultiRows( final @Nonnull long rows[], final @Nullable Object value, final @Nonnull State stateOfRows)
    {
        assert ThreadCop.enter(this, ThreadCop.ACCESS.MODIFY);
        try
        {
