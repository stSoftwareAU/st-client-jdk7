 ...
 *  @since       5 May 2010
 */
public final class HashLongMapV6 implements HashLongMap, Cloneable
{
    ...
    private static final Log LOGGER = CLogger.getLog( "com.aspc.remote.memory.impl.HashLongMapV6");//#LOGGER-NOPMD
    ...
    public HashLongMapV6(final int initialCapacity)
    {
    ...
    private void initialize()
    {
        if( table == null)
        {
            ...
            threshold = (int)(capacity * LOAD_FACTOR) + 1;
            if( count > 0)
            {
                count=0;
    ...
    public Object clone() throws CloneNotSupportedException//NOPMD
    {
        ...
        try
        {
            ...
            if( table == null)
            {
                HashLongMapV6 tempMap = new HashLongMapV6(ensureCapcity);
            ...
            HashLongMapV6 tempMap = new HashLongMapV6(table.length);
    ...
    public void putMultiRows( final long rows[], final Object value, final State stateOfRows)
    {
        ...
        try
        {
            ...
            assert stateOfRows == stateOfRows.UNKNOWN || rows.length < 2 || rows[0] != rows[1]: alert( "non-unique rows passed into putMultiRows " + rows[0]);
            ...
            if( table == null)
            {
                ...
                if( stateOfRows != stateOfRows.UNKNOWN && keyArray == null)
                {
                    keyArray = rows;
